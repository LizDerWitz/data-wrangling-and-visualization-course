...
#faux is too new for my version, installing another version:
#install.packages("devtools")  
library(devtools)
#install_github("debruine/faux", version = "1.5")  # Example version
...
#starting value - it makes it reproducible 
...
res_alpha <- data_wide_likert %>%
 #select(-id) %>% (it's another option)     the positive one, is easier cause we're ascking R directly what we need 
   select(starts_with("item_")) %>%    #is not selecting a single score, but anything that starts with ... 
  psych::alpha()
...
#-> now it is shown in the summary 
...
#changing data structure -> no data is lost, the information just reshaped. You can change it through pivot
data_long <- data_wide_likert %>%
  pivot_longer(cols = starts_with("item_"),    #which columns are going to be used
               names_to = "item",       #I want a new name column to item
               values_to = "response")    #I want a new column for response 
...
- facet_wrap() is to ggplot, as group_by for dplyr is 
