...
dat_age <- data_demographics_raw |>
  ...
  mutate(age = str_remove_all(age, "[^\\d.]"),  # retains only digits (\\d) and decimal points (.)
         age = na_if(age, ""), # marking as missing data if removed
         age = as.numeric(age)) |>
...
dat_gender <- data_demographics_raw |>
  ...
  mutate(gender = str_to_lower(gender),
         gender = str_remove_all(gender, "[\\d.]"),   #The regular expression [\\d.] specifies that it should match any digit (\\d) or decimal point (.), effectively cleaning the gender values of any numerical content.
         gender = na_if(gender, ""), 
...
# <- very relevant for quality control, what information do I have? 
...
class(data_selfreport_tidied2$response) 

#-> response is still a character

#Character does not have an ordered level, it's either numeric (factor, has levels) or a character (has no levels)

temp <-  data_selfreport_tidied2 |>
mutate(response = as.numeric(response))
         
## count number of unique_ids with correct/wrong number of responses + Exclude them
data_selfreport_tidied3 <- data_selfreport_tidied2 %>%
  mutate(correct = ifelse(row_number() <= nrow(data_selfreport_raw), 
                          data_selfreport_raw$correct[row_number()], 
                          NA)) # This method adds the correct column based on the row number, filling in with NA if there are fewer rows in 
  
data_selfreport_tidied3 <- data_selfreport_tidied3 |>
  mutate(correct = ifelse(correct == 1, "right", "wrong"))
  
data_selfreport_right <- data_selfreport_tidied3 %>%
  filter(correct == "right") %>%        # Filter for "right" answers
  distinct(unique_id) %>%                # Get distinct unique IDs
  count()                                # Count the number of unique IDs

result <- data_selfreport_tidied3 %>%
filter(correct == "right")

#testtestest 
class(temp$response)

how many unique id? 
count unique_id + count_trials -> you see how many participants did how many trials 
#for mean:
#summarize does one only
#mutate creates new rows/column, adding the value to every signle row 
...
count is group_by, summarize and then n
...
#correct number: 3, wrong number: 2 
# Print the result
print(result)

## check your exclusions worked - use: mutate, group_by, summarize, _join
