...
plot_1 <- 
  ...
  geom_smooth(method = "lm") +  #to add curves
...
#ignore anything outside of what I chose (it's not setting limits!) it's excluding the points. The calculation stays the saame, but it is zooned in 
...
ggplot(data = data_after_exclusions,
  ...
  scale_x_continuous(breaks = scales::breaks_pretty(n = 13), 
                     limits = c(0.0, 1.00)) 
  
  
  theme_minimal()

  #boundary = from where the graph starts 
...
- Exercise: How to add a dashed vertical line at the sample's mean AMP score?
  ...
  scale_y_continuous(breaks = seq(0,40,5),
                     name = "frequency") +
  geom_vline(xintercept = mean(data_after_exclusions$amp_score), linetype = "dotted") 
...
#the aesthetic call doesn't have to be in the aesthetic brakets 
...
ggplot(data = summary_amp, 
  ...
  geom_linerange(aes(ymin = amp_mean - amp_se, 
                     ymax = amp_mean + amp_se))   #for error bars 
...
```{r}
summary_selfreports <- data_after_exclusions %>%
  group_by(gender) %>%
  summarize(selfreports_mean = mean(mean_self_report),
            sreports_se = plotrix::std.error(mean_self_report))


ggplot(data = summary_selfreports, 
       aes(x = gender, 
           y =selfreports_mean)) +
  geom_col(fill = "pink", # note that you can specify specific colors using hex codes or names
           color = "black", 
           width = 0.6) +
  geom_errorbar(aes(ymin = selfreports_mean - sreports_se, 
                    ymax = selfreports_mean + sreports_se), 
                width = 0.1, 
                color = "black") +
  labs(title = "Bar Plot of with Standard Errors",
       x = "Gender",
       y = "Mean Selfreports score") +
  coord_flip()+
  theme_linedraw() 
